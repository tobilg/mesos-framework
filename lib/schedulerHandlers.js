            ...
            if (self.options.restartStates.indexOf(status.state) > -1) {
                ...
                for (index = 0;index < self.launchedTasks.length;index++) {
                    ...
                    if (status.task_id.value === task.taskId) {
                        ...
                        // Check if state is TASK_KILLED and TASK_KILLED is not in restartable states array
                        if (self.options.restartStates.indexOf("TASK_KILLED") === -1 && status.state === "TASK_KILLED") {
                ...
                if (index == self.launchedTasks.length && status.reason == "REASON_RECONCILIATION") {
                    // Cleaning up unknown tasks
                    if (self.options.kill_unknown_tasks && status.state == "TASK_RUNNING") {
                    ...
                    } else if (status.state != "TASK_RUNNING" && self.options.use_zk) {
