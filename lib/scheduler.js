...
function Scheduler (options) {
    ...
    if (self.options.useZk) {
        ...
        self.zkServicePath = "/dcos-service-" + self.options.frameworkName;
            ...
            self.zkClient.getData(zkPath, null, function (error, data, stat) {
                ...
                if (error) {
                    ...
                    if (error.getCode() == zookeeper.Exception.NO_NODE) {
                        ...
                        self.logger.debug("Node " + zkPath + " doesn't exist yet. Will be created of framework launch");
                    ...
                    // Instantiate TaskHelper
                    self.taskHelper = new TaskHelper(self);
                    // Load tasks from ZooKeeper
                    self.taskHelper.loadTasks();
                } else if (!data) {
                    self.logger.error("There was a problem fetching " + zkPath + ": " + error);
                }
...
Scheduler.prototype.subscribe = function () {
    ...
    self.logger.info(self.frameworkId)
    ...
    var Subscribe = new mesos.scheduler.Call.Subscribe(
        new mesos.FrameworkInfo(
            ...
            (new mesos.FrameworkID(self.frameworkId) || null), // id -> Use existing frameworkId for reconnections!
