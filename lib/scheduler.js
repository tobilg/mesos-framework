...
Scheduler.prototype.loadFramework = function() {
    ...
    var zk_path = self.zk_service_path + "/framework-id";
    self.zk_client.once('connected', function () {
        self.zk_client.getData(zk_path, null, function (error, data, stat) {
            if (!error && data) {
                ...
                self.taskHelper = new taskHelper.TaskHelper(self);
                self.taskHelper.loadTasks();
            }
            self.subscribe();
            self.logger.info("Subscribed to Mesos Master!");
            self.on("subscribed", function (obj) {
                if (error || !data) {
                    data = new Buffer(self.frameworkId.value);
                    // Seperating path creation from data save due to various client bugs.
                    self.zk_client.mkdirp(zk_path, function (error, stat) {
                        if (error) {
                            self.logger.error('Got error when creating a ZK node for the framework ID: ' + error);
                            self.options.use_zk = false;
                            self.zk_client.close();
                        }
                        self.zk_client.setData(zk_path,function (error, stat) {
                            if (error) {
                                self.logger.error('Got error when saving the framework ID on ZK: ' + error);
                                self.options.use_zk = false;
                                self.zk_client.close();
                            }
                        });
