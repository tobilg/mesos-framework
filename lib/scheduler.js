...
const MESOS_MASTER_DEFAULT_ZK_CONNECTION_STRING = MASTER_MESOS + ":2181";
...
function Scheduler (options) {
    ...
    self.options = {};
    self.options.frameworkName = (options.frameworkName || "mesos-js." + uuid.v4()).replace(/ /g, "-");
    ...
    self.options.zkUrl = options.zkUrl || MESOS_MASTER_DEFAULT_ZK_CONNECTION_STRING;
...
Scheduler.prototype.loadFramework = function() {
    ...
    self.options.useZk = true;
    self.zkClient = zookeeper.createClient(self.options.zkUrl);
    self.zkServicePath = SERVICE_ROOT_PATH_PREFIX + self.options.frameworkName;
            ...
            self.on("subscribed", function (obj) {
                if (error || !data) {
                    ...
                    self.zkClient.mkdirp(zkPath, function (error, stat) {
                        ...
                        self.zkClient.setData(zkPath,function (error, stat) {
                            if (error) {
