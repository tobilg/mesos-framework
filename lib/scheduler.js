...
const SERVICE_ROOT_PATH_PREFIX = "/dcos-service-";
const MASTER_MESOS = "master.mesos";
const MESOS_MASTER_ZK_CONNECTION_STRING = MASTER_MESOS + ":2181";
...
function Scheduler (options) {
    ...
    self.zk_client = null;
    self.options.use_zk = options.use_zk || false;
    self.options.kill_unknown_tasks = options.kill_unknown_tasks || false;
...
Scheduler.prototype.loadFramework = function() {
    var self = this;

    self.options.use_zk = true;
    self.zk_client = zookeeper.createClient(MESOS_MASTER_ZK_CONNECTION_STRING);
    self.zk_service_path = SERVICE_ROOT_PATH_PREFIX + self.options.frameworkName;
    var zk_path = zk_service_path + "/framework-id";
    self.zk_client.once('connected', function () {
        self.zk_client.getData(zk_path, null, function (error, data, stat) {
            if (!error && data) {
                self.logger.debug("Framework ID:" + data.toString());
                self.frameworkId = new Mesos.FrameworkID(data.toString());
                self.taskHelper = new taskHelper.TaskHelper(scheduler);
                self.taskHelper.loadTasks();
            }
            self.subscribe();
            self.logger.info("Subscribed to Mesos Master!");
            this.on("subscribed", function (obj) {
                if (error || !data) {
                    data = new Buffer(self.frameworkId.value);
                    self.zk_client.mkdirp(zk_path, data, function (error, stat) {
                        if (error) {
                            self.logger.error('Got error when setting data: ' + error);
                            self.options.use_zk = false;
...
Scheduler.prototype.sync = function () {
    var self = this;
    for (var i = 0; i < reconcileTasks.length; i++) {
        self.reconcile(self.reconcileTasks[i].task_id, self.reconcileTasks[i].runtimeInfo.agent_id);
    }
    for (var i = 0; i < self.killTasks.length; i++) {
        self.kill(self.killTasks[i].task_id, self.killTasks[i].runtimeInfo.agent_id);
    }
    if (self.use_zk)
        self.reconcile();
}
