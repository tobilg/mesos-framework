...
var mesos = new (require("./mesos"))().getMesos();
...
function Scheduler (options) {
    ...
    // Resources
    self.options.resources = options.resources; //(options.resources ? helpers.stringifyEnumsRecursive(options.resources) : []);

    // Workloads
    self.options.containerInfo = (options.containerInfo ? helpers.stringifyEnumsRecursive(options.containerInfo) : null);
    self.options.commandInfo = (options.commandInfo ? helpers.stringifyEnumsRecursive(options.commandInfo) : null);
    self.options.executorInfo = (options.executorInfo ? helpers.stringifyEnumsRecursive(options.executorInfo) : null);
    ...
    // Runtime info
    self.desiredInstances = options.instances || 1;
    self.requestedInstances = 0;
    self.runningTasks = {};
 ...
 * @returns {Object} The running task map.
 */
